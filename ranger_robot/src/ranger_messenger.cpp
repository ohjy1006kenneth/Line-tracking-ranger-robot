...
namespace westonrobot
{
  ...
  ///////////////////////////////////////////////////////////////////////////////////
  RangerROSMessenger::RangerROSMessenger(rclcpp::Node::SharedPtr &node) : pid_(0.5, 0.01, 0.1)
  {
  ...
  void RangerROSMessenger::SetupSubscription()
  {
    ...
    // depth_camera_sub_ = node_->create_subscription<sensor_msgs::msg::Image>(
    // "/camera/depth/image_raw", 10, std::bind(&RangerROSMessenger::DepthCameraCallback, this, std::placeholders::_1));

    pid_sub_ = node_->create_subscription<std_msgs::msg::Float64>(
        "/pid_deviation", 10, std::bind(&RangerROSMessenger::pid_callback, this, std::placeholders::_1));
  }
  ...
  void RangerROSMessenger::pid_callback(std_msgs::msg::Float64::SharedPtr msg)
  {

    double message = msg->data;
    RCLCPP_INFO(node_->get_logger(), "Received error: %f", message);

    if (message == 20000.0)
    {
      robot_->SetMotionMode(RangerInterface::MotionMode::kDualAckerman);
      robot_->SetMotionCommand(0.2, -0.697, 0.0);
    }
    else
    {
      double err_calculate = pid_.calculate(message);
      double steering_angle = err_calculate * 0.0035;
      robot_->SetMotionMode(RangerInterface::MotionMode::kDualAckerman);
      robot_->SetMotionCommand(0.3, steering_angle, 0);
      RCLCPP_INFO(node_->get_logger(), "Calculated error: %f", err_calculate);
    }
  }

  void RangerROSMessenger::detect_callback(std_msgs::msg::Bool::SharedPtr msg)
  {
    if (msg->data == true)
    {
      RCLCPP_INFO(node_->get_logger(), "obstacle detected: %d", msg->data);
      robot_->SetMotionCommand(0.0, 0.0); // Stop the rover
    }
  }
